...
<head>
    ...
    <title>BYSAP: <?php echo $_GET['k'] ?></title>
...
<body>
    ...
    //Display the list of products based on given keyword
    $k = ucFirst($_GET['k']);
    if (isset($_GET['shop'])) {
        $query = "SELECT * FROM bysap.Product p, bysap.Shop s WHERE s.shop_no = p.shop_no AND (product_name LIKE '%$k%' OR category LIKE '%$k%') AND shop_name = '$_GET[shop]'";
    } else {
        $query = "SELECT * FROM bysap.Product WHERE (product_name LIKE '%$k%' OR category LIKE '%$k%')";
    }
    ...
    //Display the list of products of selected range 
    if (isset($_POST['five'])) {
    ...
    if (isset($_POST['arrival'])) {
        ...
        if (isset($_GET['shop'])) {
            $query = "SELECT review.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product, bysap.Shop, bysap.Review WHERE product.shop_no = shop.shop_no AND product.product_id = review.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') AND shop_name = '$_GET[shop]' GROUP BY review.product_id, product_name, price, stock, discount, category ORDER BY avg(rating) DESC";
        } else {
            $query = "SELECT review.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product, bysap.Review WHERE product.product_id = review.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') GROUP BY review.product_id, product_name, price, stock, discount, category ORDER BY avg(rating) DESC";
        }
    }
    ...
    if (isset($_POST['fourstar'])) {
        if (isset($_GET['shop'])) {
            $query = "SELECT r.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product p, bysap.Shop s, bysap.Review r WHERE s.shop_no = p.shop_no AND p.product_id = r.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') AND shop_name = '$_GET[shop]' GROUP BY r.product_id, product_name, price, stock, discount, category HAVING avg(rating) >= 4";
        } else {
            $query = "SELECT r.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product p, bysap.Review r WHERE p.product_id = r.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') GROUP BY r.product_id, product_name, price, stock, discount, category HAVING avg(rating) >= 4";
        }
    } else  if (isset($_POST['threestar'])) {
        if (isset($_GET['shop'])) {
            $query = "SELECT r.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product p, bysap.Shop s, bysap.Review r WHERE s.shop_no = p.shop_no AND p.product_id = r.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') AND shop_name = '$_GET[shop]' GROUP BY r.product_id, product_name, price, stock, discount, category HAVING avg(rating) >= 3";
        } else {
            $query = "SELECT r.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product p, bysap.Review r WHERE p.product_id = r.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') GROUP BY r.product_id, product_name, price, stock, discount, category HAVING avg(rating) >= 3";
        }
    } else  if (isset($_POST['twostar'])) {
        if (isset($_GET['shop'])) {
            $query = "SELECT r.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product p, bysap.Shop s, bysap.Review r WHERE s.shop_no = p.shop_no AND p.product_id = r.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') AND shop_name = '$_GET[shop]' GROUP BY r.product_id, product_name, price, stock, discount, category HAVING avg(rating) >= 2";
        } else {
            $query = "SELECT r.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product p, bysap.Review r WHERE p.product_id = r.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') GROUP BY r.product_id, product_name, price, stock, discount, category HAVING avg(rating) >= 2";
        }
    } else  if (isset($_POST['onestar'])) {
        if (isset($_GET['shop'])) {
            $query = "SELECT r.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product p, bysap.Shop s, bysap.Review r WHERE s.shop_no = p.shop_no AND p.product_id = r.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') AND shop_name = '$_GET[shop]' GROUP BY r.product_id, product_name, price, stock, discount, category HAVING avg(rating) >= 1";
        } else {
            $query = "SELECT r.product_id,product_name, price, stock, discount, category, avg(rating) FROM bysap.Product p, bysap.Review r WHERE p.product_id = r.product_id AND (product_name LIKE '%$k%' OR category LIKE '%$k%') GROUP BY r.product_id, product_name, price, stock, discount, category HAVING avg(rating) >= 1";
        }
    }
            ...
            <div class="col-auto align-self-center">
                <p class="m-0">
                    ...
                    if ($count > 0) {
                        ...
                        echo "showing $count result(s) for";
                    } else {
                    ...
                    <span class="fw-bold green-txt">"<?php if (isset($_GET['shop'])) {
                                                            echo "$_GET[shop]";
            ...
            <div class="col-xl-2 col-lg-3 col-md-4 col-sm-12 border-end border-2">
                <div class="search-options">
                    <div class="d-flex flex-column me-auto">
                        <a href="search.php?k=<?php echo $_GET['k'] ?>" class="h5 pt-3 ps-4 fw-bold d-block me-auto dec-none cat-link">
                            <?php
                            echo ucfirst($_GET['k']);
                            ?>
                        </a>
                        ...
                        <?php
                        $query1 = "SELECT DISTINCT shop_name FROM bysap.Product p, bysap.Shop s WHERE s.shop_no = p.shop_no AND (product_name LIKE '%$k%' OR category LIKE '%$k%')";
                        ...
                        if ($result1) {
                            while ($data = oci_fetch_array($result1)) {
                                echo "<a href = 'search.php?k=$_GET[k]&shop=$data[SHOP_NAME]' class='h6 ps-4 d-block me-auto ms-2 mt-3 dec-none shop-link'>$data[SHOP_NAME]</a>
                    ";
                            }
                        }
                        ?>
                    ...
                    if ($count > 0) {
                        while ($data = oci_fetch_array($result5)) {
                            echo '<div class="col-xl-3 col-lg-4 col-sm-6 col-12">
                               ...
                               <img class="card-img-top resp-img" src="images/' . $data['CATEGORY'] . '/' . $data['PRODUCT_NAME'] . '.jpg" alt="' . $data['PRODUCT_NAME'] . '"' . $data['PRODUCT_NAME'] . '">
                               </a>
                            ...
                            echo '<p class="fw-bold mb-0 mt-4"> ' . $data['PRODUCT_NAME'] . '</p>';
                            ...
                            if ($data['DISCOUNT']) {
                                ...
                                if (isset($user_id)) {
                                    ...
                                    if ($data200) {
                                        echo '<a href="add_to_wishlist.php?destination=search&k=' . $_GET['k'] . '&id=' . $data['PRODUCT_ID'] . '&cart_id=' . $data100['CART_ID'] . '"><i class="fas fa-heart fill d-inline-block ms-auto mag cursor-pointer name-' . $data['PRODUCT_ID'] . '" style = "float: right;"></i></a>';
                                    } else {
                                        echo '<a href="add_to_wishlist.php?destination=search&k=' . $_GET['k'] . '&id=' . $data['PRODUCT_ID'] . '&cart_id=' . $data100['CART_ID'] . '"><i class="far fa-heart fill d-inline-block ms-auto mag cursor-pointer name-' . $data['PRODUCT_ID'] . '" style = "float: right;"></i></a>';
                                    }
                                }
                                echo '</p></div>';
